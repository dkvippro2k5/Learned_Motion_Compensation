"""
encode.py — P-frame encoder (like OpenDVC_test_P-frame.py but PyTorch 2.x)

Usage (bám sát OpenDVC CLI):
    # Encode một frame
    python encode.py --ref data/frames/f001.png --raw data/frames/f002.png \\
                     --com results/f002_rec.png  --bin bitstreams/f002.bin  \\
                     --checkpoint checkpoints/psnr_l1024/best.pth

    # Encode toàn bộ video (GOP=10)
    python encode.py --path data/frames/ --frame 100 --GOP 10 \\
                     --checkpoint checkpoints/psnr_l1024/best.pth \\
                     --metric psnr
"""
...
def get_args():
    ...
    # Single frame mode (like OpenDVC_test_P-frame.py)
    ...
    # Video mode (like OpenDVC_test_video.py)
...
def save_frame(tensor, path):
    """Save (1, 3, H, W) or (3, H, W) tensor to PNG."""
...
def save_bitstream(bitstream_dict, path):
    """Serialize bitstream dict to binary file."""
    os.makedirs(os.path.dirname(path) or '.', exist_ok=True)
...
def encode_one_pframe(model, ref_tensor, cur_tensor, bin_path, com_path, device):
    ...
    if bin_path:
        ...
        # Estimate from motion/residual strings
...
def encode_video(model, frame_dir, out_com_dir, out_bin_dir, n_frames, gop, device):
    """
    Encode a sequence of PNG frames.
    I-frames (every GOP frames) are copied; P-frames are encoded.
    ...
    for i, fname in enumerate(frame_files):
        ...
        if i % gop == 0:
            # I-frame: copy directly (no learned I-frame codec in this version)
            import shutil
