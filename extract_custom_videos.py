"""
extract_custom_videos.py — Data preparation: extract frames from raw .mp4 clips.

Turns the raw videos in data/raw_videos/ into the (ref, cur) consecutive-frame
layout consumed by FramePairDataset / get_dataloaders:

  data/real/ref/00000.png   data/real/cur/00000.png   <- (frame_t, frame_{t+1})
  data/real/ref/00001.png   data/real/cur/00001.png
  ...

One clip is held out entirely for testing (data/real_test/), so the test
sequence is never seen during training.

Usage:
    python extract_custom_videos.py
    python extract_custom_videos.py --height 256 --width 448 --stride 1 --max_pairs 2500
"""
